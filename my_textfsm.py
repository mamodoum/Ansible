...
def main(argv=None):
  ...
  with open(args[0], 'r') as template:
    ...
    #print('FSM Template:\n%s\n' % fsm)
    enTete=[str(fsm.header)] ### ENTETE OPENPYXL RESULT
    #enTete=str(fsm.header)+ "\""
    ws10.append(enTete)  ### Header
    #print(enTete)
    if len(args) > 1:
      ...
      for line in table:
        result += str(line) + '\n'
        result2 = [str(line)] ### Resultat sous forme de liste pour l'export vers Excel
        #result2 = str(line)
        ws10.append(result2)  ### Ajout de la liste dans un nouveau Excel
      ...
      for i in range(1,500): 
        if ws10.cell(row=i, column=1).value != None:
                mon_index=len(ws10.cell(row=i, column=1).value)
                chaine=ws10.cell(row=i, column=1).value
                chaine=re.sub(r"(''*|\[|\])"," ",chaine)
                chaine=re.sub(r",,*",";",chaine)

                #for k in range(mon_index):
                 #       if ws10.cell(row=i, column=1).value[k]== "'":
                 #               chaine[k] = " "
                 #       if ws10.cell(row=i, column=1).value[k]== "[" or ws10.cell(row=i, column=1).value[k]== "]":
                 #               chaine[k] = " " 
                 #       if ws10.cell(row=i, column=1).value[k]== ",":
                 #               chaine[k] = ";"
                ...
                chaine=[str(chaine)]
                ws2.append(chaine)
  wb2.save(args[1])   ### Sauvegarde la liste sous le nom suivant

  wb11 = xlrd.open_workbook(args[1])
  sh = wb11.sheet_by_name('Sheet')
  mon_fichier = open(args[1]+".csv", "w")

  for rownum in range(sh.nrows):
      i = 0
      for e in sh.row_values(rownum):
           i += 1
           if (i == 7 or i == 8 or i == 10 or i == 11) and type(e) == float:
                e = datetime(*xlrd.xldate_as_tuple(e, wb.datemode))
           elif (i == 1) and type(e) == float:
                e = int(e)
